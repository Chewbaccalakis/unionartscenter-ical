// Shared core for the Seattle Theatre Group feeds.
//
// STG runs WordPress + Modern Events Calendar (MEC), fronted by SiteGround's
// proof-of-work bot challenge. This module does the actual scraping; the files
// in ../scrapers/ (stgpresents.js + stg-<venue>.js) are thin wrappers that call
// getEvents(), filter by venue, and render a feed.
//
// The scrape:
...
//      search, the same math a browser runs) and submit the solution to obtain
...
//
// getEvents() caches the parsed events in memory so the overall feed and the
// per-venue feeds all share a single scrape per refresh instead of hitting the
// site once per feed.
//
...
import { cleanText, createCalendar } from './calendar.js';

export const SITE_NAME = 'Seattle Theatre Group';
...
const EVENTS_TTL_MS = Number(process.env.CACHE_TTL_SECONDS || 3600) * 1000;
...
function parseLdEvents(monthHtml) {
  const events = [];
...
// Scrape every event across the date window into normalized objects. Each keeps
// a `venue` field (the raw JSON-LD location name) so feeds can filter on it.
async function scrapeEvents() {
  ...
  for (let i = 0; i < MONTHS_AHEAD; i++) {
    ...
    for (const data of parseLdEvents(monthHtml)) {
      ...
      const location = [venue, cleanText(loc.address)].filter(Boolean).join(', ') || SITE_NAME;
      ...
      events.push({
        ...
        allDay: start.allDay,
        summary: cleanText(data.name) || SITE_NAME,
        venue,
        location,
        url,
...
// --- In-memory cache shared by every STG feed (ESM modules are singletons). ---
const cache = { events: null, expires: 0, inflight: null };

// Return the scraped events, reusing a recent scrape and coalescing concurrent
// callers onto a single in-flight fetch.
export async function getEvents() {
  if (cache.events && Date.now() < cache.expires) return cache.events;
  if (cache.inflight) return cache.inflight;

  cache.inflight = scrapeEvents()
    .then((events) => {
      cache.events = events;
      cache.expires = Date.now() + EVENTS_TTL_MS;
      cache.inflight = null;
      return events;
    })
    .catch((err) => {
...
// Render a list of (already filtered) events to an iCal string.
export function render(name, events) {
...
// Build a feed's generate() that keeps only events whose venue matches `pattern`
// (or all events when no pattern is given).
export function venueFeed(name, pattern) {
  return async function generate() {
    const events = await getEvents();
    return render(name, pattern ? events.filter((e) => pattern.test(e.venue)) : events);
  };
}
